...
import plus.jdk.javassist.bytecode.annotation.StringMemberValue;
import plus.jdk.javassist.compiler.MemberResolver;
import plus.jdk.javassist.expr.ExprEditor;
import plus.jdk.javassist.expr.FieldAccess;
...
public class JvstTest4 extends JvstTestRoot {
    ...
    public void testJIRA150b() throws Exception {
        int origSize = MemberResolver.getInvalidMapSize();
        int N = 100;
        for (int k = 0; k < N; k++) {
            ClassPool pool = new ClassPool(true);
            for(int paths=0; paths<50; paths++) {
        ...
        System.gc();
        System.gc();
        int size = MemberResolver.getInvalidMapSize();
    ...
    public void testJIRA151() {
        ...
        assertTrue(Desc.getClazz("[Ljava.lang.String;") != null);
        //Thread.currentThread().setContextClassLoader(TestDescForName.class.getClassLoader());
        boolean old = Desc.useContextClassLoader;
        Desc.useContextClassLoader = true;
        assertTrue(Desc.getClazz("[Ljava.lang.String;") != null);
        Desc.useContextClassLoader = old;
    }
    ...
    public void testAnnotationLoader() throws Exception {
        ...
        a.addMemberValue("value", new StringMemberValue("file/path", cp));
